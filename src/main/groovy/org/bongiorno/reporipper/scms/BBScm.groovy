...
class BBScm extends AbstractScm {
  ...
  Set<Repository> getRepos(String prj) {
    ...
    def pages = (body.size / 10 as int) + Math.min((body.size % 10) as int, 1)
    def results = (1..pages).parallelStream().map { page ->
    ...
    }.collect(partitioningBy({ Collection.class.isAssignableFrom(it.class)}))

    results[Boolean.TRUE]?.flatten()?.collect {
      new Repository(name: it.name,
          key: it.slug, description: it.description,
          clone: it.links.clone.collectEntries { [(it.name): it.href] }
      )
    } as Set
  }
